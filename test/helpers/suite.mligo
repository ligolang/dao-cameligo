...
let create_and_vote_proposal (
    tok, dao, hash_, votes : Token_helper.originated *
    DAO_helper.originated * bytes option * (int * nat * bool)
    list
) =
    ...
    let proposal = { proposal with hash = hash_ } in
    ...
    let () = Test.set_source sender_ in
    DAO_helper.end_vote_success(dao.contr)
